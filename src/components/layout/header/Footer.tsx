    ...
    <Wrapper as="footer" containerClassName="w-full" className="pt-5 md:pt-12" bgColor="bg-bg1">
      ...
      <div className="grid grid-cols-1 pb-5 md:grid-cols-10">
        ...
        <div className="col-span-7 grid grid-cols-1 justify-center gap-1 sm:grid-cols-2 md:pl-24">
          ...
          <div className="flex flex-col gap-y-2">
            ...
            <ul>
              {footerData?.list1?.links?.map((d: any, i: number) => (
                <li key={i}>
                  <Link href={d?.href} className="transition-all duration-300 hover:pl-2 hover:font-medium hover:text-zinc-900">
                    {d?.label}
                  </Link>
                </li>
              ))}
          ...
          <div className="flex flex-col">
            ...
            <p className="mb-1.5">
              <Link href={`tel:${footerData?.contactDetails?.contactNo}`} className="flex items-center gap-2">
            ...
            <p className="flex items-center gap-2">
              <IoLocationOutline className="text-xl" /> <TextWithLineBreak text={footerData?.contactDetails?.location} />
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-5 border-t-2 border-white/30 py-5 max-md:flex-col">
        <p className="text-center text-sm">
          Copyrights © {new Date().getFullYear()} <span className="font-medium">E-commerce</span>. All rights reserved. Designed and Developed by{" "}
        ...
        {/* Socials  */}
        {footerData?.socials && (
          <div className="flex-center gap-2 text-3xl">
